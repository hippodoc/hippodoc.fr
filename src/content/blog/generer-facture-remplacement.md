---
title: "T'as remplacé, t'as facturé"
description: "Génère tes factures de remplacement en quelques clics. Simple, rapide, et zéro prise de tête avec Hippodoc."
pubDate: "2025-11-28"
author: "Dr. Ryan Goburdhun"
authorRole: "Médecin remplaçant & fondateur d'Hippodoc"
category: "Guide et Conseil"
seriesId: "divers"
episodeNumber: 3
readTime: "2 min"
cover: "/blog/facture-generation-cover.png"
tags:
  - "Guide et Conseil"
cta: "Prêt à simplifier ta facturation ?"
faq:
  - question: "Quelles sont les mentions obligatoires sur une facture de remplacement ?"
    answer: "Une facture doit contenir : **nom et coordonnées** du remplaçant, **numéro SIRET**, **date et lieu** du remplacement, **montant brut et rétrocession**, et **numéro de facture** unique."
  - question: "Comment numéroter mes factures de remplacement ?"
    answer: "La numérotation doit être **chronologique et continue**. Format recommandé : AAAA-XXXX (année + numéro séquentiel). Exemple : 2025-0001, 2025-0002..."
  - question: "Combien de temps dois-je conserver mes factures ?"
    answer: "Tu dois conserver tes factures pendant **10 ans** (obligation fiscale). Hippodoc les archive automatiquement de manière sécurisée."
  - question: "Un médecin remplaçant est-il soumis à la TVA ?"
    answer: "Non, les **actes médicaux sont exonérés de TVA**. Tu n'as pas à facturer ni déclarer de TVA pour tes remplacements médicaux."
relatedArticles:
  - "signer-contrat-remplacement"
  - "checklist-administrative-medecin-remplacant"
  - "frais-professionnels-deductibles"
---

Vous l'avez réclamée… C'est fait : vous pouvez désormais générer des factures pour vos remplacements, vacations, interim ou toute autre mission en quelques clics.

## 🎯 Simple, rapide, et zéro prise de tête

Et comme toujours avec Hippodoc, c'est pas juste "possible" : **c'est simple, rapide, et zéro prise de tête** 🙌

## ⏱️ En 30 secondes top chrono :

1️⃣ **Tu crées la facture**\
2️⃣ **Tu la gardes au chaud sur Hippodoc**\
3️⃣ **Tu la télécharges en PDF**\
4️⃣ **Tu l'envoies direct**

## ✅ Fini la galère administrative

:::tip
❌ **Plus besoin** de jongler entre Word, Excel, scan, mails…\
✅ **Tout est centralisé, propre, instantané**\
🔐 Et bien sûr, **vos données sont en sécurité**, comme toujours.
:::

## 🎁 Teste cette nouvelle fonctionnalité

✨ **Nouvelle fonctionnalité disponible dès maintenant**\
🎁 **Teste-la gratuitement pendant 30 jours**\
📲 Retrouve-la dans ton espace Hippodoc
