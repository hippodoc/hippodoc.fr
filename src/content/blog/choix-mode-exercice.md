---
title: "Médecin remplaçant : les 8 statuts clés"
description: "Libéral, salariat, collaboration, mixte… découvre les 8 modes d'exercice pour bien débuter et choisir celui qui te correspond."
pubDate: "2025-10-21"
author: "Dr. Ryan Goburdhun"
authorRole: "Médecin remplaçant & fondateur d'Hippodoc"
category: "Guide et Conseil"
seriesId: "divers"
episodeNumber: 1
readTime: "6 min"
cover: "/blog/choix-mode-exercice-cover.png"
tags:
  - "Guide et Conseil"
faq:
  - question: "Quels sont les différents statuts pour un médecin remplaçant ?"
    answer: "Les 8 statuts principaux sont : **Remplacement libéral** (60-80% des honoraires), **Salariat** (hôpital, centre de santé), **Collaboration libérale**, **Exercice mixte**, **Vacations médicales**, **Intérim médical**, **MSP** et **Télémédecine**."
  - question: "Peut-on cumuler salariat et libéral ?"
    answer: "Oui, le **statut mixte** est légal et courant. Tu peux être salarié pour la sécurité et faire du libéral pour la liberté et compléter tes revenus. Attention aux clauses d'exclusivité dans certains contrats."
  - question: "Quel statut choisir quand on débute ?"
    answer: "Pour débuter sereinement, le **salariat** offre stabilité et protection sociale complète. Le **remplacement libéral** est idéal si tu veux plus de flexibilité et un meilleur potentiel de revenus."
  - question: "Quels sont les avantages du remplacement libéral ?"
    answer: "Le remplacement libéral offre : **flexibilité totale** du planning, **rémunération attractive** (60-80% des honoraires), formation variée, pas de gestion de cabinet, et liberté géographique."
relatedArticles:
  - "salariat-vs-liberal"
  - "remplacement-salarie-guide-complet"
  - "regime-fiscal-micro-bnc-vs-reel"
---

Voici un panorama clair, rapide et concret des 8 modes d'exercice les plus fréquents 👇

## 🏥 1. Remplacement Libéral

• **Statut** : Libéral (BNC)\
• **Rémunération** : 60-80 % des honoraires\
• **Fiscalité** : [Micro-BNC ou réel](/blog/regime-fiscal-micro-bnc-vs-reel), [URSSAF](/blog/tout-comprendre-urssaf), [CARMF](/blog/tout-comprendre-carmf)

✅ **+** : Flexible, formateur, bien payé, sans gestion de cabinet\
⚠️ **-** : Revenus irréguliers

## 💼 2. Salariat (Hôpital, Centre de Santé)

• **Statut** : Salarié (CDD/CDI)\
• **Fiscalité** : Impôt à la source (pas d'URSSAF/CARMF)

✅ **+** : Stabilité, congés payés\
⚠️ **-** : Moins d'autonomie, horaires fixes, salaire plafonné

## 🤝 3. Collaboration Libérale

• **Statut** : Libéral (BNC)\
• **Spécificité** : Redevance fixe ou % au titulaire

✅ **+** : Développer sa patientèle, tremplin vers l'installation\
⚠️ **-** : Pas de revenu garanti, redevance parfois élevée

## ⚖️ 4. Exercice Mixte

• **Statut** : Libéral + Salarié\
• **Fiscalité** : Deux sources à déclarer

✅ **+** : Liberté + sécurité\
⚠️ **-** : Double gestion, charge mentale

## 🌐 5. Vacations Médicales

• **Statut** : Libéral ou salarié

✅ **+** : Missions ponctuelles, grande flexibilité\
⚠️ **-** : Statut précaire, pas de congés

## 🧳 6. Intérim Médical

• **Statut** : Salarié via agence d'intérim

✅ **+** : Bien rémunéré, aucune paperasse, planning à la carte\
⚠️ **-** : Peu de perspectives d'évolution

## 🏡 7. Maison de Santé Pluriprofessionnelle (MSP)

• **Statut** : Libéral (BNC)\
• **Particularité** : Charges partagées et déductibles

✅ **+** : Travail en équipe, impact local\
⚠️ **-** : Dépendance au collectif

## 📱 8. Télémédecine

• **Statut** : Libéral ou salarié\
• **Fiscalité** : BNC ou salaire

✅ **+** : Ultra flexible, pas de local nécessaire\
⚠️ **-** : Isolement, routine, réservé aux thésés

## 🔥 TIPS HIPPODOC.FR

:::tip
Quel que soit ton statut, Hippodoc t'aide à :

✅ Suivre tes revenus [super-net](/blog/simulateur-super-net-combien-reste) en temps réel

✅ Comparer tes différentes activités

✅ Visualiser tes statistiques clés

✅ Planifier ton activité grâce à un agenda pro intégré

📊 Une vision claire, précise et complète de ta carrière médicale. Le tout, au même endroit.

👇 Et toi, tu choisis quoi ? Partage ton expérience en commentaire !
:::
