---
title: "Checklist administrative du remplaçant"
description: "RCP, Ordre, CPAM, URSSAF sous 8 jours, CARMF dès le premier remplacement, prévoyance : quand faire chaque démarche et dans quel ordre, sans rien oublier."
pubDate: "2025-10-06"
updatedDate: "2026-09-21"
author: "Dr. Ryan Goburdhun"
authorRole: "Médecin remplaçant & fondateur d'Hippodoc"
category: "Fiche Pratique"
categoryNumber: "#3"
seriesId: "fiche-pratique"
episodeNumber: 3
readTime: "4 min"
cover: "/blog/checklist-administrative-cover.png"
tags:
  - "Fiche Pratique"
cta: "🚀 Simplifie ta gestion administrative avec Hippodoc ! Centralise tous tes documents et concentre-toi sur l'essentiel : soigner tes patients."
faq:
  - question: "L'assurance RCP est-elle obligatoire pour un médecin remplaçant ?"
    answer: "Oui, la **RCP** (Responsabilité Civile Professionnelle) est **obligatoire AVANT** ton premier remplacement. Elle te protège contre les risques professionnels. Compare les offres (MACSF, GPM, La Médicale) - certaines sont gratuites via les syndicats d'internes."
  - question: "Quand dois-je m'inscrire à l'URSSAF ?"
    answer: "Tu dois t'inscrire à l'URSSAF dans les **8 jours** suivant le début de ton activité libérale. Tu recevras ton numéro SIRET, indispensable pour la facturation."
  - question: "Puis-je bénéficier d'une dispense CARMF ?"
    answer: "Oui, si ton **revenu net d'activité indépendante reste inférieur à 15 000 €** et que tu n'es pas assujetti à la CET. La démarche n'est **pas automatique** : la dispense doit être demandée. Attention, la période dispensée ne compte pas pour tes droits à la retraite et te laisse sans couverture invalidité-décès."
  - question: "Une prévoyance est-elle nécessaire pour un jeune remplaçant ?"
    answer: "Fortement recommandée ! La prévoyance garantit ton **maintien de revenu** en cas d'arrêt maladie/accident. Souscrire jeune te permet de bénéficier de conditions avantageuses."
relatedArticles:
  - "obtenir-sa-licence-de-remplacement"
  - "tout-comprendre-urssaf"
  - "signer-contrat-remplacement"
---

Tu as enfin trouvé ton remplacement idéal ! 🎉 Avant d'enfiler ta blouse blanche et de démarrer cette nouvelle aventure, petit passage obligé par la case administrative. Mais pas de stress, voici ta checklist complète pour commencer sereinement. 📝🚀

## ✅ Responsabilité Civile Professionnelle (RCP)

📅 **Quand ?** Obligatoire AVANT ton [premier remplacement](/blog/realites-remplacement-medical).\
🧐 **Pourquoi ?** Elle te protège contre les risques professionnels comme les erreurs médicales ou incidents imprévus pouvant survenir pendant ta pratique.\
📌 **Astuce :** N'hésite pas à comparer les différentes offres proposées par les assureurs spécialisés (MACSF, GPM, La Médicale…). Certaines assurances sont même incluses gratuitement via les syndicats d'internes, alors renseigne-toi bien avant de souscrire !

## ✅ Inscription à l'Ordre des Médecins

📅 **Quand ?** Obligatoire AVANT ton premier remplacement.\
🧐 **Pourquoi ?** L'inscription au tableau de l'Ordre est une obligation légale pour exercer en France. Elle officialise ton droit à pratiquer et te permet de [facturer](/blog/generer-facture-remplacement) tes actes.\
📌 **Astuce :** Anticipe cette démarche car les délais peuvent varier selon les départements. Prévois plusieurs semaines d'avance pour éviter tout retard dans le début de ton activité.\
🎓 **Si tu es encore interne :** ce n'est pas l'inscription au tableau qu'il te faut, mais la [licence de remplacement](/blog/obtenir-sa-licence-de-remplacement) — même principe, mêmes délais à anticiper.

## ✅ Inscription à la CPAM

📅 **Quand ?** Dès ton tout premier remplacement.\
🧐 **Pourquoi ?** Cela permet à l'Assurance Maladie d'être informée officiellement de ta situation professionnelle.\
📌 **Astuce :** Aucune carte à réclamer à la CPAM : ta carte CPS (ou CPF si tu es interne sous licence) t'est envoyée automatiquement par l'Agence du numérique en santé après ton enregistrement à l'Ordre. C'est elle qui te sert à télétransmettre les feuilles de soins électroniques.

## ✅ Inscription à l'URSSAF

📅 **Quand ?** Dans les 8 jours suivant le début effectif de ton activité libérale — [le délai officiel](https://entreprendre.service-public.gouv.fr/vosdroits/F36740) pour t'immatriculer au guichet des formalités des entreprises, remplacements compris.\
🧐 **Pourquoi ?** Cela te permet de déclarer officiellement ton activité professionnelle indépendante, et tu recevras ainsi ton numéro SIRET, indispensable pour la facturation.\
📌 **Astuce :** Profite de l'espace simplifié [RSPM](/blog/rspm-exemples-concrets) pour déclarer en ligne tes revenus et payer tes cotisations facilement ([URSSAF](/blog/tout-comprendre-urssaf) et [CARMF](/blog/tout-comprendre-carmf)). Il est réservé aux remplacements exclusifs, dans la limite de [19 000 € d'honoraires rétrocédés par année civile](https://entreprendre.service-public.gouv.fr/vosdroits/R63763).

## ✅ Inscription à la CARMF

📅 **Quand ?** Dès le début de ton activité libérale. L'affiliation est obligatoire pour tout médecin rémunéré à l'honoraire, remplacements compris — il n'y a pas de durée minimale d'activité en dessous de laquelle tu en serais exempté.\
🧐 **Pourquoi ?** La Caisse Autonome de Retraite des Médecins de France (CARMF) gère ta retraite complémentaire obligatoire et d'autres couvertures spécifiques.\
📌 **Astuce :** Tu peux demander une **dispense totale d'affiliation** si ton revenu net d'activité indépendante reste inférieur à **15 000 €** et que tu n'es pas assujetti à la contribution économique territoriale (CET). [Cette dispense n'est pas automatique](https://www.carmf.fr/page.php?page=cdrom/coti/coti-affiliation.htm) : elle doit être demandée.

:::warning
La dispense réduit tes charges, mais elle a un prix : pendant la période dispensée, tu n'es plus couvert contre le risque invalidité-décès, et ces périodes ne compteront jamais dans le calcul de tes droits à la retraite. À arbitrer en connaissance de cause, pas par défaut.
:::

## ✅ Prévoyance

📅 **Quand ?** Dès que possible, idéalement dès le début de ta carrière professionnelle.\
🧐 **Pourquoi ?** Une assurance prévoyance garantit ton maintien de revenu en cas d'arrêt de travail prolongé pour cause de maladie ou d'accident.\
📌 **Astuce :** Souscrire jeune à une prévoyance te permet de profiter de conditions avantageuses spécialement conçues pour les jeunes médecins.

## 💡 L'astuce Hippodoc

:::tip
Centralise et importe tous tes documents administratifs importants directement dans l'application Hippodoc. Fini les documents égarés et les heures passées à chercher, tout est désormais accessible facilement depuis ton smartphone ! 🌟
:::
